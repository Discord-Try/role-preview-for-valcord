...
export function RoleConfigurator(props: RoleConfiguratorProps) {
    ...
    const nativeRoles = props.roles.filter((role) => role.native);
    const customRoles = props.roles.filter((role) => !role.native);

    return (
        <div className="RoleConfigurator">
            <div className="roles-list native">
                {nativeRoles.map((role) => (
                    <div className="role">
                        <div className="options">
                            <div className="color-button-wrapper">
                                <div className="color-button" style={{ ["--role-color" as any]: role.color }}>
                                    {role.color}
                                </div>
                            </div>
                            <span className="name-input">{role.name}</span>
                        </div>
                    </div>
                ))}
            </div>
